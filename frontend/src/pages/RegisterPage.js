...
const RegisterPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.password !== formData.confirmPassword) {
      setError(language === 'en' ? 'Passwords do not match' : 'Hasła nie są zgodne');
      return;
    }

    if (formData.password.length < 6) {
      setError(language === 'en' ? 'Password must be at least 6 characters' : 'Hasło musi mieć co najmniej 6 znaków');
      return;
    }
    ...
    try {
      ...
      toast.success(t('success'), { 
        description: language === 'en' 
          ...
          : 'Sprawdź swoją skrzynkę email aby znaleźć kod weryfikacyjny'
      });
    ...
    <div className="min-h-screen bg-[#09090b] flex">
      ...
      <div className="hidden lg:block lg:w-1/2 relative">
        ...
        <div className="absolute bottom-12 left-12 right-12">
          ...
          <h3 className="text-white text-2xl font-bold mb-2">{t('createAccount')}</h3>
          <p className="text-zinc-300 text-sm">
            {language === 'en' 
              ? 'Join the digital identity system and access all government services'
              : 'Dołącz do systemu tożsamości cyfrowej i uzyskaj dostęp do wszystkich usług rządowych'
            }
          </p>
        </div>
      </div>
      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-center px-8 lg:px-16 py-12">
        ...
        <div className="max-w-md mx-auto w-full">
          ...
          {/* Welcome text */}
          <div className="mb-8">
            <h2 className="text-white text-3xl font-bold mb-2">{t('createAccount')}</h2>
            <p className="text-zinc-400">
              {language === 'en' 
                ? 'Create your digital identity account'
                : 'Utwórz swoje konto tożsamości cyfrowej'
              }
            </p>
          </div>
          ...
          <p className="mt-8 text-center text-zinc-400">
            {t('hasAccount')}{' '}
            <Link to="/login" data-testid="login-link" className="text-white hover:underline font-medium">
              {t('signIn')}
            </Link>
          </p>
        </div>
